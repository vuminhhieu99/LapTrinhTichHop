...
namespace QLNCKHGV.Controllers
{
    public class SachController : ApiController
    {
        ...
        [Route("api/Sach/GetAllSearch")]
        public IHttpActionResult GetSearch(string textSearch, string nam, int? ki, int pageNumber, int pageSize)
        {
            IList<SachModel> listItems = null;
            if (textSearch == null)
            {
            ...
            else
            {
                ...
                listItems = db.Saches.Where(k => k.Ma.ToLower().Contains(textSearch) || k.Ten.ToLower().Contains(textSearch))
                .Select(k => new SachModel()
                {
                    Id = k.Id,
                    Ma = k.Ma,
        ...
        public IHttpActionResult PutSua(Sach model)
        {
            ...
            if (dbEntry == null)
            {
                return Ok(new { status = false, message = "Không tồn tại Sách" });
            }
