...
class DetaiJS {
    ...
    loadData() {
        ...
        var self = this;
        $.ajax({
            url: 'http://localhost:54555/api/BaiBao/GetAllSearch',
            ...
            $.each(data.items, function (index, item) {
                table.row.add([
                    ...
                    item.Ma,
                    item.Ten,
                    item.LoaiBaiBao == null ? "" : item.LoaiBaiBao.Ten,
                    item.TenTapChiCongBo,
                    item.SoThanhVien,
                    item.NamHoc,
                    item.KiHoc,
                    `<button type="button" class="btn btn-default edit" data-id="${item.Id}" data-idloaddt="${item.LoaiBaiBao == null ? "0" : item.LoaiBaiBao.Id}">
    ...
    loadLoaiDT(select, selectID = 0) {
        ...
        $.ajax({
            url: 'http://localhost:54555/api/BaiBao/GetLoaiBB',
            dataType: 'json',
            method: 'GET'
        }).done(function (data) {
    ...
    getByID(id_Detai) {
        $.ajax({
            ...
            $('#EditForm input[name="CoQuanQuanLy"]').val(data.TenTapChiCongBo);
            $('#EditForm select[name="NamHoc"]').val(data.NamHoc);
            $('#EditForm select[name="KiHoc"]').val(data.KiHoc);

            $('#EditDeTai').data('id', data.Id);
            var selectID = data.LoaiBaiBao == null ? "0" : data.IdLoaiBaiBao;
...
$('#confirmDel').click(function () {
    ...
    $.ajax({
        url: 'http://localhost:54555/api/BaiBao/XoaBaiBao?Id=' + id_Detail,
        method: "DELETE",
        dataType: 'json',
...
$('#CreateDeTai').click(function () {
    ...
    model.IdLoaiBaiBao = $('#createForm select[name="IdLoaiDeTai"]').val();
    model.TenTapChiCongBo = $('#createForm input[name="CoQuanQuanLy"]').val();
    model.NamHoc = $('#createForm select[name="NamHoc"]').val();
    model.KiHoc = $('#createForm select[name="KiHoc"]').val();

    $.ajax({
        url: 'http://localhost:54555/api/BaiBao/ThemBaiBao',
...
$('#EditDeTai').click(function () {
    ...
    model.IdLoaiBaiBao = $('#EditForm select[name="IdLoaiDeTai"]').val();
    model.TenTapChiCongBo = $('#EditForm input[name="CoQuanQuanLy"]').val();
    model.NamHoc = $('#EditForm select[name="NamHoc"]').val();
    model.KiHoc = $('#EditForm select[name="KiHoc"]').val();

    $.ajax({
        url: 'http://localhost:54555/api/BaiBao/SuaBaiBao',
        data: JSON.stringify(model),
        dataType: 'json',
        method: 'PUT',
        contentType: 'application/json'
...
$('tbody').delegate("td", "click", function () {
    ...
    if (child.length == 0) {
        ...
        window.location.href = "/BaiBao/Detail?id=" + id;
    }
    return true;
});